...
class EditAo extends Component
{
    ...
    public $currentStep = 1,
        ...
        //section_2
        $select_partie_administratif = [],
        $select_partie_financiaire = [],
        $select_partie_technique = [],

        //partie_3
        $adresse, $listLocations = [];


    public function mount($ao){
        $this->aoObj = $ao;

        //dd($this->aoObj->partenariats);
        foreach($this->aoObj->partenariats as $partenariat){
            $this->partenariat_id[] = $partenariat->id;
        }

        foreach($this->aoObj->utilisateurs as $select_partie_administratif){
            $this->select_partie_administratif[] = $select_partie_administratif->id;
        }


        foreach($this->aoObj->bus as $select_partie_financiaire){
            $this->select_partie_financiaire[] = $select_partie_financiaire->id;
        }

        foreach($this->aoObj->departements as $select_partie_technique){
            $this->select_partie_technique[] = $select_partie_technique->id;
        }

        $this->setChefDeFilOptions();

        $this->u_id = $ao->id;
        $this->num_AO = $this->aoObj->num_AO;
        $this->date_limite = $this->aoObj->date_limite;
        $this->date_de_depot = $this->aoObj->date_de_depot;
        $this->pays_id = $this->aoObj->pays_id;
        $this->type_id = $this->aoObj->type_id;
        $this->adjudication_id = $this->aoObj->adjudication_id;
        $this->ministere_id = $this->aoObj->ministere_id;
        $this->secteur_id = $this->aoObj->secteur_id;
        //$this->partenariat_id = $this->aoObj->partenariat_id;
        $this->chef_de_fil_id = $this->aoObj->chef_de_fil_id;
        $this->montant_soumission = $this->aoObj->montant_soumission;
        $this->budget = $this->aoObj->budget;
        $this->n_lot = $this->aoObj->n_lot;
        $this->client_id = $this->aoObj->client_id;
        $this->montant_c_p = $this->aoObj->montant_c_p;
        $this->critere_selection_id = $this->aoObj->critere_selection_id;
        $this->objet = $this->aoObj->objet;
        $this->RC = $this->aoObj->RC;
        $this->CPS = $this->aoObj->CPS;
        $this->AVIS = $this->aoObj->CPS;
        //$this->geom = null;
        //$this->select_partie_administratif = $this->aoObj->select_partie_administratif;
        //$this->select_partie_financiaire = $this->aoObj->select_partie_financiaire;
        //$this->select_partie_technique = $this->aoObj->select_partie_technique;
        //$this->listLocations = [];
    }
    ...
    public function firstStepSubmit()
    {
        ...
        $this->validate([
            'num_AO' => 'required|unique:aos,num_AO,'.$this->u_id,
...
//        ]);
    ...
    public function render()
    {
        ...
        return view('livewire.edit-ao', $data);
    }
}
