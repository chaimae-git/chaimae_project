...
class PieceAPreparerAdministratif extends Component
{
    ...
    {
        //->orWhere('ao_id',$this->ao)
        //$pieces_a_preparer = Piece_a_preparer::whereNull('ao_id')->orWhere('ao_id',$this->ao)->where('partie_id', $this->partie)->orderBy('nom_fichier')->get();
        $pieces_a_preparer = Piece_a_preparer::where(function($query){$query->whereNull('ao_id')->where('partie_id', $this->partie);})->orWhere(function($query){$query->where('ao_id',$this->ao)->where('partie_id', $this->partie);})->orderBy('id', 'DESC')->get();
        //dd($pieces_a_preparer);
